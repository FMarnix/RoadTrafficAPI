...
class RoadSegmentTests(APITestCase):
    ...
    def test_update_road_segment(self):
        url = reverse(
            "roadsegment-detail", kwargs={"pk": self.road_segment.pk}
        )  # Endpoint for updating road segments
        data = {"name": "Updated Road Segment"}
        response = self.client.put(url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            RoadSegment.objects.get(pk=self.road_segment.pk).name,
            "Updated Road Segment",
        )

    def test_delete_road_segment(self):
        url = reverse(
            "roadsegment-detail", kwargs={"pk": self.road_segment.pk}
        )  # Endpoint for deleting road segments
...
class TrafficSpeedTests(APITestCase):
    ...
    def test_update_traffic_speed(self):
        url = reverse(
            "trafficspeed-detail", kwargs={"pk": self.traffic_speed.pk}
        )  # Endpoint for updating traffic speeds
        data = {
            "road_segment": self.road_segment.pk,
    ...
    def test_delete_traffic_speed(self):
        url = reverse(
            "trafficspeed-detail", kwargs={"pk": self.traffic_speed.pk}
        )  # Endpoint for deleting traffic speeds
        response = self.client.delete(url)
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
...
class ObservationTests(APITestCase):
    def setUp(self):
        ...
        self.sensor = Sensor.objects.create(
            name="Test Sensor", uuid="270e4cc0-d454-4b42-8682-80e87c3d163c"
        )
        self.road_segment = RoadSegment.objects.create(name="Test Road Segment")
